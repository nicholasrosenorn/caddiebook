...
- **Request limits (defense-in-depth, `server/src/middleware/body-limit.ts`).** Every route group caps its request body before the handler buffers it (over-limit → `413`): `/data` 512 KiB (a full embedded-holes round PUT is a few KB), `/auth` · `/community` · `/notifications` 32 KiB, and legacy `/sync` 32 MiB (the one-time migration flushes a whole local DB in one un-chunked POST). Free-text fields are also length-capped (over-cap → `400`): `/data` writes enforce it centrally in `upsertRow`/`patchHole` (`service.ts`) — long-form `notes`/`body`/settings `value` ≤ 10 000 chars, all other text (names, clubs, review/goal answers, tags) ≤ 1 000; profile (`auth/routes.ts`) caps `firstName`/`lastName` ≤ 100 and `avatar` ≤ 256 (it's a short icon-name id). When you add a new free-text column, it inherits the `/data` cap automatically via the allowlist; pick the long-form tier only if it holds prose or a serialized blob.
...
| `/(tabs)/(profile)` (Me tab) | **Built.** A single sectioned surface: a top section rail + a horizontal `pagingEnabled` pager. **Page 0 = Overview** — the editorial Me summary (`components/me-summary-view.tsx`, `MeSummaryViewBase`): handicap hero + sparkline, skill cards, putting bars, ledger, personal bests. Its cards **jump the pager** to the matching deep section (via `onNavigateSection`) rather than pushing a screen. **Pages 1–6** are the deep sections (`SectionContent` in `app/(tabs)/(profile)/index.tsx`): Scoring, Driving, Approach, Short, Mental, Bests — across **completed** rounds. The controller owns the shared filters (a compact chip → panel: hole count All/18/9 default 18, recency last 20/40/60/All) applied to every page, plus the deep-page club filters. Each section: an editorial `SectionHero` (numeral + `TrendChart` sparkline) over boxed figure tiles (`FiguresRow boxed`) and bar charts; aggregate driver/approach dispersion (muted pins), split distance bars, review insights. Logic in `lib/lifetime-stats.ts`; one cached read via `useStatsBundle()` (`lib/data/stats.ts`). |

...
/**
 * Derive the strokes-gained inputs for one hole from the round's raw data. The
 * pure SG math lives in lib/strokes-gained.ts; here we resolve the end-state of
 * the approach (on-green + proximity, from the mapped approach shot or the GIR
 * flag) and the first-putt distance. Returns null for holes that can't be
 * decomposed — no approach distance, no putt count, or a blocked green.
 */
function holeSGInput(
  hole: Hole,
  approachShot: Shot | undefined,
  holePutts: Putt[],
  ctx: SGContext,
): HoleSGInput | null {
  if (
  ...
  const firstPuttFt = holePutts.length
    ? Math.max(...holePutts.map((p) => p.distanceFt))
    : null;
...
/**
 * Visit each decomposable hole of a round, yielding its strokes-gained breakdown
 * and the resolved inputs (so callers can bucket by approach/putt distance). The
 * one place that builds the approach-shot + putts-by-hole maps and runs the pure
 * per-hole SG math; `computeRoundSG` and the band aggregation both go through it.
 */
function eachHoleSG(
  holes: Hole[],
  putts: Putt[],
  shots: Shot[],
  ctx: SGContext,
  visit: (sg: HoleSG, input: HoleSGInput) => void,
): void {
  const approachByHole = new Map<number, Shot>();
  for (const s of shots) {
    if (s.shotType === 'approach') approachByHole.set(s.holeNumber, s);
  }
  const puttsByHole = new Map<number, Putt[]>();
  for (const p of putts) {
    const arr = puttsByHole.get(p.holeNumber);
    if (arr) arr.push(p);
    else puttsByHole.set(p.holeNumber, [p]);
  }

  for (const hole of holes) {
    const input = holeSGInput(
      hole,
      approachByHole.get(hole.holeNumber),
      puttsByHole.get(hole.holeNumber) ?? [],
      ctx,
    );
    if (!input) continue;
    const sg = holeStrokesGained(input);
    if (sg) visit(sg, input);
  }
}

/** Strokes gained summed across one round's holes (vs the PGA Tour baseline). */
export function computeRoundSG(
  ...
  eachHoleSG(holes, putts, shots, ctx, (sg) => {
    acc = addHoleSG(acc, sg);
  });
  return acc;
}

/** A distance band's pooled SG, normalized per 18 (so bands sum to the matching
 *  category bar), plus the hole count behind it (drives the baseline split). */
export type SGBandRow = { key: string; label: string; sgPer18: number; holes: number };

export type StrokesGainedStats = {
  /** Pooled SG sums across every round in the set. */
  sg: RoundStrokesGained;
  /** Chronological (oldest→newest) per-round total SG, projected per 18. */
  trend: number[];
  /** Approach SG by approach-distance band (`APPROACH_BUCKETS`), per 18 vs Tour. */
...
/**
 * Strokes gained across a set of rounds, plus the per-round total trend. The
 * driver distance (for the Off-the-Tee hole-length estimate) comes from the
 * player's bag/handicap and is passed in via `ctx`.
 */
export function aggregateStrokesGained(
  ...
  for (const r of rounds) {
    eachHoleSG(
      holesByRound.get(r.id) ?? [],
      puttsByRound.get(r.id) ?? [],
      shotsByRound.get(r.id) ?? [],
      ctx,
      (sg, input) => {
        pooled = addHoleSG(pooled, sg);

        // Approach band by remaining yardage (last bucket is open-ended).
        const ai = APPROACH_BUCKETS.findIndex((b) => input.approachDistanceYds < b.max);
        const aBand = approachSum[ai === -1 ? approachSum.length - 1 : ai];
        aBand.sg += sg.approach;
        aBand.holes += 1;

        // Putt band by first-putt (lag) distance; holes with no putt logged
        // (firstPuttFt null → putting SG 0) contribute nothing and sit in no band.
        if (input.firstPuttFt != null) {
          const pi = PUTT_BUCKETS.findIndex((b) => input.firstPuttFt! <= b.ft);
          const pBand = puttSum[pi === -1 ? puttSum.length - 1 : pi];
          pBand.sg += sg.putting;
          pBand.holes += 1;
        }
      },
  ...
  // Normalize every band by the single per-18 factor so the bands sum to the
  // card's APPR / PUTT category bars.
  const f = pooled.holesWithSG > 0 ? 18 / pooled.holesWithSG : 0;
  const approachBands: SGBandRow[] = APPROACH_BUCKETS.map((b, i) => ({
    key: b.label,
    label: b.label,
    sgPer18: approachSum[i].sg * f,
    holes: approachSum[i].holes,
  }));
  const puttingBands: SGBandRow[] = PUTT_BUCKETS.map((b, i) => ({
    key: String(b.ft),
    label: b.label,
